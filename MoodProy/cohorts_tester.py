...
def test_add_cohort_members(test):
    ...
    #  Adds a user to a cohort manually
        ...
        show(t.add_cohort_members([{'cohorttype':{'type':raw_input(),'value':raw_input()},'usertype':{'type':raw_input(),'value':raw_input()}}]))
    elif test == '1':
    # Add a member using user and cohort IDs
        show(t.add_cohort_members([{'cohorttype':{'type':'id','value':'1'},'usertype':{'type':'id','value':'3'}}]))
    elif test == '2':
    # Add a member using username and cohort idnumber
        show(t.add_cohort_members([{'cohorttype':{'type':'idnumber','value':'35'},'usertype':{'type':'username','value':'student1'}}]))
    elif test == '3':
    # Get a warning because cohort specified do not exist
        show(t.add_cohort_members([{'cohorttype':{'type':'id','value':'1100'},'usertype':{'type':'id','value':'3'}}]))
...
def test_delete_cohort_members():
    ...
    show(t.delete_cohort_members([{'cohortid':raw_input(),'userid':raw_input()}]))
...
def test_update_cohorts(test):
    ...
    #  Create 1 cohort manually
        ...
        show(t.update_cohorts([{'type':raw_input(),'value':raw_input(),'name':raw_input(),'idnumber':raw_input()}]))
    elif test == '1':
    # Update 1 cohort
        show(t.update_cohorts([{'id':4,'type':'id','value':'1','name':'Python Cohort','idnumber':'35','description':'This cohort has been created with the MoodLib python library.'}]))
    elif test == '2':
    # Update 1 cohort
        show(t.update_cohorts([{'id':4,'type':'id','value':'2','name':'Python Cohort Changed','idnumber':'36','description':'This cohort has been changed with the MoodLib python library.'}]))
    elif test == '3':
    # Update 1 cohort
        show(t.update_cohorts([{'id':1,'type':'system','value':'1','name':'Cohort1', 'idnumber':''}]))
    # Error
    elif test == '4':
        show(t.update_cohorts([{'id':4,'type':'id','value':'1','idnumber':'36'}]))
