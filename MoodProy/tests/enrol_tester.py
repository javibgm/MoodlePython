...
def test_get_enrolled_users_with_capability(test):
    ...
    #  Get all enrolled users' info from 1 course manually with a student capability
        ...
        show(t.get_enrolled_users_with_capability([{'courseid':raw_input(),
                                                    'capabilities':['moodle/blog:view']}
                                                   ]))
    elif test == '1':
    #  Get enrolled users from 1 course with a student capability (options: show only fullname)
        show(t.get_enrolled_users_with_capability([{'courseid':2,'capabilities':['moodle/blog:view']}],
                                                  [{'name':'userfields','value':'fullname'}]))
    elif test == '2':
    #  Get enrolled users from 1 course with a manager capability (options: show only fullname)
        show(t.get_enrolled_users_with_capability([{'courseid':2,'capabilities':['enrol/authorize:managepayments']}],
                                                  [{'name':'userfields','value':'fullname'}]))
    #Errors
    elif test == '3':
        show(t.get_enrolled_users_with_capability([{'courseid':2}]))
    elif test == '4':
        show(t.get_enrolled_users_with_capability([{'courseid':-1,
                                                    'capabilities':['moodle/blog:view']}]))
    elif test == '5':
        show(t.get_enrolled_users_with_capability([{'courseid':2,'capabilities':[]}]))
    elif test == '6':
        show(t.get_enrolled_users_with_capability([{'courseid':2,'capabilities':['moodle/blog:view']}],
                                                  [{'name':'userfields','value':'nofileds'}]))
...
def test_manual_enrol_users(test):
    ...
    #  Enroll a user in a course with specified role manually
        ...
        show(t.manual_enrol_users([{'roleid':raw_input(),
                                    'userid':raw_input(),
                                    'courseid':raw_input()}]))
    elif test == '1':
    #  Enroll a user in a course with specified role manually, starts in 30 seconds and ends in 2 mins
        ...
        show(t.manual_enrol_users([{'roleid':raw_input(),
                                    'userid':raw_input(),
                                    'courseid':raw_input(),
                                    'timestart':int(time.time())+30,
                                    'timeend':int(time.time())+150}]))
    #  Errors
    elif test == '2':
        show(t.manual_enrol_users([{'roleid':-1,
                                    'userid':4,
                                    'courseid':4}]))
    elif test == '3':
        show(t.manual_enrol_users([{'roleid':1,
        ...
        show(t.manual_enrol_users([{'userid':4,
                                    'courseid':4}]))
    elif test == '6':
        show(t.manual_enrol_users([{'roleid':1,
                                    'userid':4,
